#!/usr/bin/env python3
"""
Quick launcher for visualizing the hard dataset.

This script:
1. Checks if hard data is downloaded
2. Launches the WebSocket streaming server
3. Launches the web viewer server
4. Opens the viewer in your default browser

Usage:
    python scripts/visualize_hard.py

    # Or via uv
    uv run python scripts/visualize_hard.py

    # With custom ports
    python scripts/visualize_hard.py --ws-port 8765 --web-port 8000

    # Don't open browser automatically
    python scripts/visualize_hard.py --no-browser
"""
...
def check_hard_data(data_dir: Path) -> bool:
    """Check if hard data is downloaded."""
    data_file = data_dir / "hard" / "track2_data.parquet"
    return data_file.exists()


def load_hard_data(data_dir: Path) -> tuple[np.ndarray, float]:
    """Load hard dataset."""
    data_path = data_dir / "hard" / "track2_data.parquet"
...
def print_header() -> None:
    ...
    console.print(
        Panel.fit(
            "[bold red]Hard Dataset Viewer[/bold red]\n"
            "[dim]Challenging conditions - matches live evaluation[/dim]",
            border_style="red",
        )
    )
    console.print()
...
def print_instructions(web_host: str, web_port: int, ws_port: int) -> None:
    ...
    table.add_row("Dataset:", "hard (challenging - matches live eval)")

    console.print(Panel(table, title="[bold green]Servers Running[/bold green]", border_style="green"))
    console.print()
    console.print("[yellow]Note:[/yellow] This dataset has noise, artifacts, and bad channels")
...
def main(
    ...
    """
    Launch the hard dataset viewer.

    This starts both the WebSocket streaming server and the web viewer,
    then optionally opens your browser to visualize the challenging dataset
    that matches live evaluation conditions.
    """
    print_header()

    # Check if data exists
    if not check_hard_data(data_dir):
        console.print("[red]Error:[/red] hard dataset not found!")
        console.print()
        console.print("Download it with:")
        console.print("  [cyan]uv run python -m scripts.download hard[/cyan]")
        console.print()
        raise typer.Exit(code=1)

    console.print("[green]✓[/green] Found hard dataset")
    console.print()

    # Load data
    try:
        data, fs = load_hard_data(data_dir)
