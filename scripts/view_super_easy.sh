#!/usr/bin/env bash
# Quick launcher for viewing the super_easy dataset
#
...
#
#   # Or with uv
#   bash scripts/view_super_easy.sh
...
echo ""
echo -e "${CYAN}Super Easy Dataset Viewer${NC}"
echo -e "${DIM}Crystal-clear signals for understanding neural activity${NC}"
echo ""

# Check if data exists
if [ ! -f "$DATA_DIR/track2_data.parquet" ]; then
    echo -e "${RED}Error:${NC} super_easy dataset not found!"
    echo ""
    echo "Download it with:"
    echo -e "  ${CYAN}uv run python -m scripts.download super_easy${NC}"
    echo ""
    exit 1
fi

echo -e "${GREEN}✓${NC} Found super_easy dataset"
echo ""
...
echo -e "  ${CYAN}Dataset:${NC}          super_easy (crystal-clear signals)"
echo ""
